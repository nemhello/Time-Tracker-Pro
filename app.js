...
function setupEventListeners() {
    ...
    document.getElementById('viewLocationLogBtn').addEventListener('click', showLocationLog);
...
async function logCurrentLocation() {
    const btn = document.getElementById('floatingLogBtn');
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
        ...
        try {
            ...
            const data = await res.json();
            entry.address = data.display_name || `${latitude.toFixed(5)}, ${longitude.toFixed(5)}`;
        } catch {
            entry.address = `${latitude.toFixed(5)}, ${longitude.toFixed(5)}`;
        }

        saveLocationLog();
        // Refresh table if log view is open
        if (!document.getElementById('locationLogView').classList.contains('hidden')) {
            renderLocationLog();
        }
...
function renderLocationLog() {
    const container = document.getElementById('locationLogTable');
    const subtitle = document.getElementById('logSubtitle');

    if (locationLog.length === 0) {
        subtitle.textContent = '';
        container.innerHTML = '<div class="no-entries">No taps logged yet. Use the 📍 button to log your location.</div>';
        return;
    }

    subtitle.textContent = `${locationLog.length} tap${locationLog.length !== 1 ? 's' : ''} recorded`;
    ...
    container.innerHTML = `
        <table class="log-table">
            ...
            <tbody>
                ${locationLog.map(entry => {
                    const d = new Date(entry.timestamp);
                    const timeStr = d.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false });
                    const dateStr = d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
                    const isToday = d.toDateString() === new Date().toDateString();
                    const displayDate = isToday ? timeStr : `${dateStr} ${timeStr}`;
                    const shortAddr = entry.address.length > 60 ? entry.address.substring(0, 60) + '…' : entry.address;

                    return `
                    <tr>
                        <td class="log-time">${displayDate}</td>
                        <td class="log-addr" title="${entry.address}">
                            <a href="https://maps.apple.com/?ll=${entry.lat},${entry.lng}" target="_blank">${shortAddr}</a>
                        </td>
                        <td class="log-acc">${entry.accuracy}m</td>
                        <td><button class="btn-delete" onclick="deleteLogEntry(${entry.id})">&#128465;</button></td>
                    </tr>`;
                }).join('')}
            </tbody>
        </table>
        <button class="btn btn-secondary" style="margin-top:16px;" onclick="clearAllLogEntries()">&#128465; Clear All</button>
    `;
}
